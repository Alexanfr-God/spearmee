...
export function RegistrationWizard({
  ...
  const toggleLang = (code: string) => {
    ...
    set({
      native_languages: cur.includes(code) ? cur.filter((c) => c !== code) : [...cur, code],
    });
  };
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-[420px] flex-col">
      ...
      <main className="flex-1 overflow-x-hidden px-4 py-4">
          ...
          >
            ...
            {steps[step].key === "basics" && (
              <>
                ...
                <Field
                  label={t("onboarding.basics.birth")}
                  hint={age ? t("common.years", { count: age }) : undefined}
                >
                  <Input
                    type="date"
              ...
              <>
                ...
                <Field label={t("onboarding.appearance.eyeColor")}>
                  <OptionGrid
                    group="eye_color"
                    values={EYE_COLORS}
                    selected={form.eye_color}
                    onSelect={(v) => set({ eye_color: v })}
                  />
                </Field>
                <Field label={t("onboarding.appearance.hairColor")}>
                  <OptionGrid
                    group="hair_color"
                    values={HAIR_COLORS}
                    selected={form.hair_color}
                    onSelect={(v) => set({ hair_color: v })}
                  />
                </Field>
              ...
              <>
                <Field label={t("onboarding.intent.wantsChildren")}>
                  <OptionGrid
                    ...
                    values={WANTS_CHILDREN}
                    columns={4}
                    selected={form.wants_children}
                    onSelect={(v) => set({ wants_children: v })}
                  />
                </Field>
                <Field label={t("onboarding.intent.childrenTimeline")}>
                  <OptionGrid
                    group="children_timeline"
                    values={CHILDREN_TIMELINE}
                    columns={2}
                    selected={form.children_timeline}
                    onSelect={(v) => set({ children_timeline: v })}
                  />
                </Field>
                <Field label={t("onboarding.intent.hasChildren")}>
                  <OptionGrid
                    ...
                    onSelect={(v) => set({ has_children: v })}
                  />
                </Field>
                <Field label={t("onboarding.intent.relationshipGoal")}>
                  <OptionGrid
                    group="relationship_goal"
                    values={RELATIONSHIP_GOAL}
                    columns={2}
                    selected={form.relationship_goal}
                    onSelect={(v) => set({ relationship_goal: v })}
                  />
                </Field>
              </>
            )}
              ...
              <>
                ...
                <Field label={t("onboarding.lifestyle.exercise")} optional>
                  <OptionGrid
                    ...
                    onSelect={(v) => set({ exercise: v })}
                  />
                </Field>
                <Field label={t("onboarding.lifestyle.wantsMarriage")}>
                  <OptionGrid
                    group="wants_marriage"
                    values={WANTS_MARRIAGE}
                    selected={form.wants_marriage}
                    onSelect={(v) => set({ wants_marriage: v })}
                  />
                </Field>
                <Field label={t("onboarding.lifestyle.willingToRelocate")}>
                  <OptionGrid
                    ...
                    selected={form.willing_to_relocate}
                    onSelect={(v) => set({ willing_to_relocate: v })}
                  />
                </Field>
              </>
            )}
